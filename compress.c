...
/*void create_freq_array(lli* frequence) { // inicializar na main e excluir aqui
    for (int i = 0; i < 256; i++) {
        frequence[i] = 0;
    }
}*/
...
void compact_file(FILE* arq_compact, HASH* hash, uchar trash_size) {
    ...
    while (fscanf(read_file, "%c", &caracter) != EOF) { // lemos o arquivo ate o EOF.
        ...
        size = hash -> array[caracter] -> size; // pega o tamanho dessa codificaçao.
